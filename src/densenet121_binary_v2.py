# src/train_densenet_improved.py
...
# Data
# --------------------
def collect_split(split_root: Path) -> Tuple[list, list]:
    img_paths, labels = [], []
    cls2idx = {"benign": 0, "malignant": 1}
    for cls in cls2idx:
...
class FibroDataset(torch.utils.data.Dataset):
...
train_ds = FibroDataset(DATA_ROOT / "train", transform=train_tfms)
val_ds   = FibroDataset(DATA_ROOT / "val",   transform=eval_tfms)
test_ds  = FibroDataset(DATA_ROOT / "test",  transform=eval_tfms)
